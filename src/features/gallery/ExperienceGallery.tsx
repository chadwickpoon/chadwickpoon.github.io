...
export function ExperienceGallery({ experiences }: { experiences: readonly Experience[] }) {
  ...
  const ordered = state.interest === "all" ? experiences : [
    ...experiences.filter((entry) => entry.category === state.interest),
    ...experiences.filter((entry) => entry.category !== state.interest),
  ];
  return <section id="explore" className="gallery-section">
    <div className="section-heading"><div><p className="eyebrow">PICK A DOOR</p><h2 id="explore-heading" tabIndex={-1}>A few worlds to explore.</h2></div><span className="collection-count" aria-live="polite">{experiences.length} things to wander into</span></div>
    <div className="experience-grid">
      {ordered.map((entry) => <Link key={entry.slug} href={experiencePath(entry.slug)} className={`experience-card card-${entry.slug}`}>
